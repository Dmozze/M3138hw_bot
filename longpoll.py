...
def checkUserName(username):
    return username != None and (username.lower() == TEACHER_USERNAME or username.lower() == ADMIN_USERNAME)
...
def help(bot, update):
    if checkUserName(update.message.from_user.username):
        ...
        update.message.reply_text(
            ...
            'Добавить можно только нерешенные задания\n'
...
def add(bot, update, args):
    with shelve.open('names_db',writeback=True) as shelve_names:
        ...
        toadd = good_set(args)
        if len(toadd) > 50:
            update.message.reply_text('Слишком много задач, не находите?')
            return
        with shelve.open('private_db') as shelve_private:
             toadd = (toadd & set(shelve_private['unsolved_tasks']))
...
def update(bot,update):
    if not checkUserName(update.message.from_user.username):
        update.message.reply_text('У вас недостаточно прав для выполнения указанной команды')
        return
    unsolved_tasks = upload()
    with shelve.open('private_db') as shelve_private:
        shelve_private['unsolved_tasks'] = unsolved_tasks
    data = []
    with shelve.open('names_db') as shelve_names:
        for tasknumber in unsolved_tasks:
...
def reset(bot, update):
    ...
    with shelve.open('names_db', writeback=True) as shelve_names:
        for i in shelve_names.keys():
            temp_list = []
            for task in shelve_names[i]['tasks']:
                if task in unsolved_tasks:
                    temp_list.append(task)
            shelve_names[i]['tasks'] = temp_list
